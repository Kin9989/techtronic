...
class Product(models.Model):
    ...
    def get_average_rating(self):
        average_rating = self.productrating_set.aggregate(Avg("rating_value"))[
            "rating_value__avg"
        ]
    ...
    def get_price(self):
        formatted_price = (
            f"{self.price:,.3f}".replace(",", "X").replace(".", ",").replace("X", ".")
        )
...
class CartItem(models.Model):
    ...
    def calculate_total_cart(user):
        ...
        formatted_price = (
            f"{total_price:,.3f}".replace(",", "X").replace(".", ",").replace("X", ".")
        )
        return formatted_price
...
class Order(models.Model):
    ...
    payment_method = models.CharField(
        max_length=50, choices=METHODS_CHOICES, blank=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def get_formatted_total_amount(self):
        ...
        formatted_price = (
            formatted_price.replace(",", "X").replace(".", ",").replace("X", ".")
        )
        return formatted_price
...
class OrderItem(models.Model):
    ...
    def get_product_sales_by_day(start_date=None, end_date=None):
        ...
        completed_orders = Order.objects.filter(
            status="Completed", created_at__range=(start_date, end_date)
        )
        sales_data = (
    ...
    def get_daily_revenue_by_year(start_date=None, end_date=None):
        ...
        yearly_revenue_data = (
            completed_orders.annotate(year=ExtractYear("created_at"))
            .values("year")
            .annotate(total_revenue=Sum("total_amount"))
            .order_by("year")
        )
